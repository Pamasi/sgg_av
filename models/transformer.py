...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model:int=512, nhead:int=8, num_encoder_layers:int=6,
                 num_decoder_layers:int=6, dim_feedforward:int=2048, dropout:int=0.1,
    ...
    def forward(self, src, mask, entity_embed, triplet_embed, pos_embed, so_embed):
        ...
        entity_embed_ps, entity = torch.split(entity_embed, c, dim=1)
        triplet_embed_ps, triplet = torch.split(triplet_embed, [c, 2 * c], dim=1)

            
        entity_embed = entity_embed_ps.unsqueeze(1).repeat(1, bs, 1)
        triplet_embed = triplet_embed_ps.unsqueeze(1).repeat(1, bs, 1)
        ...
        # permute in order to be multiple with KGE
        feature_context =  memory.permute(1, 0, 2)
        
        return hs.transpose(1, 2), hs_t.transpose(1, 2), \
                so_masks, memory.permute(1, 2, 0).view(bs, c, h, w)
                
                #feature_context, entity_embed_ps, triplet_embed_ps
