...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True, use_log=False):
        ...
        if self.enable_ltn==True:
            ...
            if self.use_tgt_constr==True:
                ...
                losses['sat_agg_error'] =  1 - self._compute_sat_level(outputs, sub_logits, obj_logits,  target_sub_classes, 
                                                                    target_obj_classes, rel_target_classes,  rel_target_classes[rel_idx][idx_2],  rel_logits[rel_idx][idx_2], use_log=use_log)
                ...
                losses['sat_agg_error'] =  1 - self._compute_sat_level(outputs, sub_logits, obj_logits, sub_class,
                                                                       obj_class, rel_class, rel_target_classes[rel_idx][idx_2],  rel_logits[rel_idx][idx_2], use_log=use_log)
    ...
    def _compute_sat_level(self, outputs:Dict[str,torch.Tensor], sub_logits:torch.Tensor, obj_logits:torch.Tensor, target_sub_classes:torch.Tensor, 
                           target_obj_classes:torch.Tensor, rel_target_classes:torch.Tensor, ground_truth_rel:Optional[torch.Tensor]=None, rel_to_verify:Optional[torch.Tensor]=None, use_log=False):
        """
        ...
        if sub_valid_cls.size()[0]>0 and obj_valid_cls.size()[0]>0 and  rel_valid_cls.size()[0]>0: 
            ...
            if self.rc_enable:
                ...
                if rel_to_verify is not None and  ground_truth_rel is not None and rel_to_verify.size(0)>0 and ground_truth_rel.size(0)>0:
                    ...
                    has_rc = True
                    rel_axiom = self.eval_rel_constr(rel_to_verify, ground_truth_rel, use_log=use_log)
            ...
            if self.ec_enable:  
                entity_axiom = self.eval_entity_constr(sub_boxes,  obj_boxes,
                                                       ...
                                                       sub_valid_cls, obj_valid_cls, use_log=use_log)
                ...
                if self.constr_neg_type==0:
                    if self.equal_p:
                        neg_axiom = self.eval_neg_constr_v1(sub_boxes,  obj_boxes,
                                                        ...
                                                        sub_valid_cls, obj_valid_cls, rel_valid_cls, use_log=use_log)
                    ...
                    if self.equal_p:
                        neg_axiom = self.eval_neg_constr_v2(sub_boxes,  obj_boxes,
                                                    sub_logits, obj_logits, rel_logits,
                                                    sub_valid_cls, obj_valid_cls, rel_valid_cls, forall_type=0, use_log=use_log)
                        if self._save_sat_debug:
                            self.sat_debug['neg_axiom_sat'] = self.sat_neg_axiom(neg_axiom)
                        else:
                            self.sat_debug['neg_axiom_sat'] = -1.0
                        ...
                        if torch.sum(rel_easy_idx)>0:
                            ...
                            neg_easy_axiom = self.eval_neg_constr_v2(sub_boxes,  obj_boxes,
                                                        sub_logits, obj_logits, rel_logits,
                                                        sub_valid_cls, obj_valid_cls, rel_easy_n_valid_cls, forall_type=0, use_log=use_log)
                        ...
                        if torch.sum(rel_hard_idx)>0:
                            ...
                            neg_hard_axiom = self.eval_neg_constr_v2(sub_boxes,  obj_boxes,
                                                        sub_logits, obj_logits, rel_logits,
                                                        sub_valid_cls, obj_valid_cls, rel_hard_n_valid_cls, forall_type=1, use_log=use_log)
                ...
                if self.equal_p:
                    ...
                    if torch.sum(rel_easy_idx)>0:
                        ...
                        pos_easy_axiom = self.eval_pos_constr(sub_boxes,  obj_boxes,
                                                        sub_logits, obj_logits, rel_logits,
                                                        sub_valid_cls, obj_valid_cls, rel_easy_p_valid_cls, forall_type=0, use_log=use_log)
                    ...
                    if torch.sum(rel_hard_idx)>0:
                        ...
                        pos_hard_axiom = self.eval_pos_constr(sub_boxes,  obj_boxes,
                                                        sub_logits, obj_logits, rel_logits,
                                                        sub_valid_cls, obj_valid_cls, rel_hard_p_valid_cls, forall_type=1, use_log=use_log)
    ...
    def eval_entity_constr(self, sub_boxes, obj_boxes, sub_logits, obj_logits, sub_valid_cls, obj_valid_cls, use_log=True):
    ...
    def eval_rel_constr(self, rel_logits, rel_gt_cls, use_log=True):
        """
    ...
    def eval_neg_constr_v1(self,  sub_boxes, obj_boxes, sub_logits, obj_logits,  rel_logits, sub_valid_cls, obj_valid_cls, rel_valid_cls, use_log=True):
        """
        ...
        gr_sub, gr_obj, gr_rel , mask_idx_tensor, constr_tensor = grounding(out_sub_box, out_obj_box ,
                                                                                ...
                                                                                use_regressor=self.use_regressor, use_log=use_log)
    ...
    def eval_pos_constr(self, sub_boxes, obj_boxes,  sub_logits, obj_logits,  rel_logits, sub_valid_cls, obj_valid_cls, rel_valid_cls, forall_type, use_log=True):
        """
        evaluated the negative constraints axiom in a fuzzy-fashion
    ...
    def eval_neg_constr_v2(self, sub_boxes, obj_boxes,  sub_logits, obj_logits,  rel_logits, sub_valid_cls, obj_valid_cls, rel_valid_cls, forall_type, use_log=True):
        """
        evaluated the negative constraints axiom in a fuzzy-fashion
        forall_type(bool): 0 easy or differentiation among axiom difficulty, 
                           1 hard constraints
