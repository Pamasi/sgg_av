...
class BasicSceneGraphEvaluator:
    ...
    def evaluate_scene_graph_entry(self, gt_entry, pred_scores, background_first, viz_dict=None, iou_thresh=0.5):
        res = evaluate_from_dict(gt_entry, pred_scores, self.mode, self.result_dict, background_first,
...
def evaluate_from_dict(gt_entry, pred_entry, mode, result_dict, background_first, multiple_preds=False,
    ...
    :param result_dict: 
    :param background_first first element is related to background relationship class
    :param viz_dict: 
    :param kwargs: 
    :return: 
    """
    ...
    # 1 plus because there <no-relationship> label
    # for the relationship query to the embedding
    ...
    for k in result_dict[mode + '_recall']:
        # return the unique sorted union inside the prediction array
...
def evaluate_recall(gt_rels, gt_boxes, gt_classes,
    ...
    # produc all entity score i.e. score_head*score_tail*score_predicates 
...
def _compute_pred_matches(gt_triplets, pred_triplets,
    ...
    gt_has_match = keeps.any(1) # keeps all true prediction
...
def calculate_mR_from_evaluator_list(evaluator_list, mode, multiple_preds=False, save_file=None, 
    ...
    # to account for class not present in the test-set
    ...
    # change to account for relationship not present in the dataset
    rel_num = 0
    ...
    if verbose:
        if multiple_preds:
            ...
            #  constraint that an object x cannot be in relationship with itself
    ...
    return mean_recall, all_rel_results
