...
def box_cxcywh_to_xyxy(x: torch.Tensor):
    ...
    # assert sum([ 1 if i==False else 0 for i in (x_c +w)<2048]) ==0, 'All box witdh should be less than 2048 '
    # assert sum([ 1 if i==False else 0 for i in (y_c +h)<1024]) ==0, 'All box height should be less than 2048 '
...
# modified from torchvision to also return the union
def box_ir(boxes1: torch.Tensor, boxes2: torch.Tensor):
    """
    calculate inclusion ration
    """
    area1 = box_area(boxes1)
    ...
    # 1:1 rel betwen box1 and  box2
    ir = inter.diag() / area1
    return ir.unsqueeze(1)



def box_centroid(boxes: torch.Tensor) -> torch.Tensor:
...
def get_union_box(boxes1: torch.Tensor, boxes2: torch.Tensor):
    """
